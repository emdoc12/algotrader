...
import { Plus, Trash2, Settings2, Zap, ZapOff, FlaskConical, Radio, X } from "lucide-react";
import { useLocation } from "wouter";
import type { Strategy, Account, WatchlistItem } from "@shared/schema";
...
function WatchlistSection({ strategyId }: { strategyId: number }) {
  const { toast } = useToast();
  const [newSymbol, setNewSymbol] = useState("");

  const { data: items = [] } = useQuery<WatchlistItem[]>({
    queryKey: ["/api/watchlist", strategyId],
    queryFn: () => apiRequest("GET", `/api/watchlist/${strategyId}`).then((r) => r.json()),
    refetchInterval: 5000,
  });

  const addMutation = useMutation({
    mutationFn: (symbol: string) =>
      apiRequest("POST", "/api/watchlist", { strategyId, symbol: symbol.toUpperCase().trim() }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/watchlist", strategyId] });
      setNewSymbol("");
    },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["/api/watchlist", strategyId] }),
  });
  ...
  return (
    <div className="bg-muted/50 rounded-md p-3 space-y-2">
      <span className="text-xs font-medium text-muted-foreground">Watchlist (symbols to trade)</span>
      <div className="flex flex-wrap gap-1 min-h-[24px]">
        {items.length === 0 && (
          <span className="text-xs text-amber-400">⚠ Empty — add symbols below to start trading</span>
        )}
        {items.map((item) => (
          <span
            key={item.id}
            className="flex items-center gap-1 bg-background border border-border rounded px-2 py-0.5 text-xs font-mono"
          >
            {item.symbol}
            <button
              onClick={() => deleteMutation.mutate(item.id)}
              className="text-muted-foreground hover:text-destructive"
              data-testid={`button-remove-symbol-${item.id}`}
            >
              <X className="h-3 w-3" />
            </button>
          </span>
      ...
      <div className="flex gap-2">
        <Input
          data-testid={`input-symbol-${strategyId}`}
          placeholder="e.g. BTC/USD or ETH"
          value={newSymbol}
        ...
        <Button
          ...
          data-testid={`button-add-symbol-${strategyId}`}
        >
            ...
            <Card key={s.id} data-testid={`card-strategy-${s.id}`}>
              ...
              <CardContent className="space-y-3">
                ...
                {["crypto_momentum", "crypto_mean_reversion"].includes(s.type) && (
                  <WatchlistSection strategyId={s.id} />
                )}
                {["crypto_momentum", "crypto_mean_reversion"].includes(s.type) && (
                  <Button
                    variant="outline"
